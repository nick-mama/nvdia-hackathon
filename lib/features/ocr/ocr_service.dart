...
class OcrService {
  ...
  /// Build a correctly formatted InputImage for iOS ML Kit
  Future<InputImage?> _buildInputImage(Uint8List imageBytes) async {
    final decodedImage = img.decodeImage(imageBytes);
    if (decodedImage == null) return null;

    // Convert to BGRA format (required for iOS ML Kit)
    final bgraBytes = Uint8List(decodedImage.width * decodedImage.height * 4);
    int i = 0;
  ...
  Future<String?> recognizeText(Uint8List imageBytes) async {
    ...
    try {
      final inputImage = await _buildInputImage(imageBytes);
      if (inputImage == null) {
