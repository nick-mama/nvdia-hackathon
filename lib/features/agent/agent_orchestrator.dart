...
enum AgentTool {
  describeScene('describe_scene', 'Analyze camera frame and describe the environment'),
  readText('read_text', 'Extract and read text from the camera view'),
  flagHazard('flag_hazard', 'Alert user to immediate danger'),
  queryRag('query_rag', 'Query knowledge base for location-specific information'),
  estimateRoute('estimate_route', 'Provide navigation guidance'),
  analyzeDocument('analyze_document', 'Deep analysis of documents or complex text');
...
class AgentOrchestrator {
  ...
  Map<String, dynamic> _buildPerceptionPacket(AgentContext context) {
    return {
      ...
      'detections': context.detections.map((d) => {
        return {
          'class': d.className,
  ...
  String _buildNavigationGuidance(List<DetectionResult> detections) {
    ...
    // Group by position
    final ahead = detections.where((d) => d.spatialPosition == 'ahead').toList();
    final left = detections.where((d) => d.spatialPosition == 'left').toList();
    final right = detections.where((d) => d.spatialPosition == 'right').toList();
    
    if (ahead.isNotEmpty) {
      buffer.write('${ahead.first.className} ${ahead.first.distanceFeet.toStringAsFixed(0)} feet ahead. ');
    }
